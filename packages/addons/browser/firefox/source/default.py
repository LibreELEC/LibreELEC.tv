...
def _print_exception():
  ...
  xbmc.log(f'## Firefox Addon Error: in ({filename}, line {lineno}\n"{line.strip()}"):\n{repr(exc_obj)}', xbmc.LOGERROR)

def startfirefox(args):
  try:
    firefox_env = {
    ...
    firefox_env['AUDIO_DEVICE_TYPE'] = getAudioDeviceType(audio_device)
    if firefox_env['AUDIO_DEVICE_TYPE'] == 'ALSA':
      alsa_device = getAlsaAudioDevice(audio_device)
      firefox_env['ALSA_DEVICE'] = alsa_device if alsa_device else ''

    # firefox user-agent string
    if __addon__.getSetting('USE_CUST_USERAGENT') == 'true':
      firefox_env['USER_AGENT'] = __addon__.getSetting('CUST_USERAGENT_STR')

    # launch firefox
    new_env = os.environ.copy()
    new_env.update(firefox_env)
    firefox_execute = ([os.path.join(__path__, 'firefox-start')]
                      + args
                      + [__addon__.getSetting('HOMEPAGE')])
    subprocess.call(firefox_execute, env=new_env)
  except Exception:
    _print_exception()
    xbmc.log(f'## Firefox Addon Error: firefox env: {firefox_env}, audio device: {audio_device}', xbmc.LOGERROR)
...
if args == 'widevine':
  ...
  if not isRunning('firefox'):
    pauseXbmc()
    startfirefox(args)
    while isRunning('firefox'):
      time.sleep(1)
    resumeXbmc()
